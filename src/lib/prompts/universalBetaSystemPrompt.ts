...
**중요**: 웰컴 메시지에서는 게임 개수(365개 등), 데이터베이스 규모, 전문가 수준 등의 통계적 정보나 성능 관련 주장을 절대 포함하지 마세요.
...
5. **간결성**: 웰컴 메시지에서 게임 개수, 데이터베이스 규모, 통계 정보는 언급하지 말 것
...
**절대 금지**: 게임 개수, 데이터베이스 규모, 전문가 수준 주장 등 통계 정보 언급 금지
`;
        } else {
            return baseContext + `

...
// autoevals가 사용할 수 있도록 Gemini API를 래핑하는 함수
...
// autoevals 초기화 - OpenAI 호환 클라이언트 생성
...
const EVALUATION_CONFIG = {
  ...
  timeoutMs: 60000,       // API 호출 타임아웃 (30초 -> 60초로 증가)
...
async function evaluateQuestion(testCase) {
  ...
  try {
    ...
    try {
      // Faithfulness 평가 - 답변이 제공된 컨텍스트에 기반하는지 평가
    ...
    try {
      // Answer Relevancy 평가 - 답변이 질문과 얼마나 관련있는지 평가
    ...
    try {
      // Context Recall 평가 - 검색된 컨텍스트가 이상적인 컨텍스트를 얼마나 포함하는지 평가
    ...
    try {
      // Context Precision 평가 - 검색된 컨텍스트의 정확성 평가
  ...
  try {
    ...
    const evaluationResult = {
      ...
      model_config: {
        ...
        // llm_model을 gpt-4 대신 gemini-pro로 변경합니다.
